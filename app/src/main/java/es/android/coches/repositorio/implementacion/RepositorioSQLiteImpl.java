...
public class RepositorioSQLiteImpl extends SQLiteOpenHelper implements Repositorio<Pregunta> {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE " + ContratoPregunta.EntradaPregunta.NOMBRE_TABLA +
                " (" + ContratoPregunta.EntradaPregunta._ID +
                " INTEGER PRIMARY KEY AUTOINCREMENT," + ContratoPregunta.EntradaPregunta.NOMBRE +
                " TEXT NOT NULL," + ContratoPregunta.EntradaPregunta.FOTO +
                " TEXT NOT NULL," + "UNIQUE (" + ContratoPregunta.EntradaPregunta.NOMBRE + "))");
    ...
    public List<Pregunta> getAll() {
        ...
        Cursor c = db.query(ContratoPregunta.EntradaPregunta.NOMBRE_TABLA,// Nombre de la tabla
        ...
        while (c.moveToNext()) {
            @SuppressLint("Range") String nombre = c.getString(c.getColumnIndex(ContratoPregunta.EntradaPregunta.NOMBRE));
            @SuppressLint("Range") String foto = c.getString(c.getColumnIndex(ContratoPregunta.EntradaPregunta.FOTO));
    ...
    public void update(Pregunta pregunta) {
        ...
        values.put(ContratoPregunta.EntradaPregunta.FOTO, pregunta.getFoto());
        // Actualizar...
        db.update(ContratoPregunta.EntradaPregunta.NOMBRE_TABLA, values, "nombre=?",
    ...
    public void delete(Pregunta pregunta) {
        SQLiteDatabase db = getWritableDatabase();
        db.delete(ContratoPregunta.EntradaPregunta.NOMBRE_TABLA, "nombre=?",
                new String[]{pregunta.getNombre()}); // arraypara sustituir ? por valores
    }


}

class ContratoPregunta {/* contructor privado parar que no se pueda instanciar la clase accidentalmente */
